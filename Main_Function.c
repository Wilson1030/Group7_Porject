...
void ReadCourse()
{
	...
	CourseCount = i;
	/*while (fgets(course, sizeof(course), fp) != NULL)
	{
		fgets(course[i].name, sizeof(course[i].name), fp);
		for (i = 0; i < CourseCount; i++)
		{
			printf("%d.\t%s\t%s", i + 1, course[i].code, course[i].name);
		}
	}
	CourseCount = i;*/
	fclose(RC);
